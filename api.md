...
| [options.badRejection]           | <code>Boolean</code>                        | <code>false</code>                                | Set to `true` **if and only if** the Node.js version you're using terminates the process on unhandled rejections. This option is only for _forward-compatibility purposes_.                                                                                                                                                                 |
...
Use this method to upload a file with a sticker for later use in _createNewStickerSet_ and _addStickerToSet_ methods (can be used multiple
times).
...
You must use exactly one of the fields _png_sticker_, _tgs_sticker_, or _webm_sticker_
...
| name          | <code>String</code>                                                      | Short name of sticker set, to be used in `t.me/addstickers/` URLs (e.g., _"animals"_). Can contain only english letters, digits and underscores. Must begin with a letter, can't contain consecutive underscores and must end in `"_by_<bot_username>"`. `<bot_username>` is case insensitive. 1-64 characters. |
...
an [Update](https://core.telegram.org/bots/api#update) with the field _pre_checkout_query_.
